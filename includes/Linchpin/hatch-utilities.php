...
class HatchUtilities {
	...
	static function breadcrumbs() {
		...
		<?php if ( ! is_home() ) { ?>
			...
			<li><a href="<?php esc_attr_e( get_option( 'home' ) ); ?>"><?php esc_html_e( 'Home', 'propane' ); ?></a></li>
...
?>
	...
	static function comment_nav() {
		...
		<nav class="navigation comment-navigation" role="navigation">
			<h2 class="screen-reader-text"><?php _e( 'Comment navigation', 'propane' ); ?></h2>
			<div class="nav-links">
				<?php
					if ( $prev_link = get_previous_comments_link( __( 'Older Comments', 'propane' ) ) ) :
						printf( '<div class="nav-previous">%s</div>', $prev_link );
					endif;

					if ( $next_link = get_next_comments_link( __( 'Newer Comments', 'propane' ) ) ) :
