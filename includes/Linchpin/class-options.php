...
class Options {
	...
	public function theme_options_add_page() {
		...
		$theme_page = add_theme_page(
			esc_html__( 'Additional Options', 'propane' ),
			esc_html__( 'Additional Options', 'propane' ),
			'edit_theme_options',
			'theme_options',
			...
			<h2 class="nav-tab-wrapper">
				...
				class="nav-tab <?php echo esc_attr( $active_tab_class ); ?>">
					...
					printf(
						esc_html(
							...
							__( '%s Additional Footer Content', 'propane' )
						),
				...
				class="nav-tab <?php echo ( 'script_options' === $active_tab ) ? 'nav-tab-active' : ''; ?>">
					...
					printf(
						esc_html(
							// translators: 1. blog Name
							__( '%s Additional Scripts', 'propane' )
						),
						$current_theme->get( 'Name' )
					);
					?>
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( ! empty( $scripts ) ) {
			foreach ( $scripts as $key => $script ) {
				if ( ! empty( $script[0] ) ) {
					...
					wp_register_script( $key, get_stylesheet_directory_uri() . $script[0], $dependencies, PROPANE_VERSION, true );
		...
		if ( ! empty( $styles ) ) {
			foreach ( $styles as $key => $style ) {
				if ( ! empty( $style[0] ) ) {
					wp_register_style( $key, get_stylesheet_directory_uri() . $style[0], array(), PROPANE_VERSION );
