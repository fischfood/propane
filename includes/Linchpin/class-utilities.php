...
class Utilities {
	...
	public static function breadcrumbs() {
		...
		?>
			...
			sprintf( esc_html__( 'Archive for %s', 'propane' ), the_time( 'F jS, Y' ) );
			?>
			</li>
		<?php } elseif ( is_month() ) { ?>
			<li>
			...
			sprintf( esc_html__( 'Archive for %s', 'propane' ), the_time( 'F, Y' ) );
			?>
			</li>
		<?php } elseif ( is_year() ) { ?>
			<li>
			...
			sprintf( esc_html__( 'Archive for %s', 'propane' ), the_time( 'Y' ) );
			?>
		<?php } elseif ( is_author() ) { ?>
			<li><?php esc_html__( 'Author Archive', 'propane' ); ?></li>
		<?php } elseif ( isset( $_GET['paged'] ) && ! empty( $_GET['paged'] ) ) { // WPCS ignore. ?>
			<li><?php esc_html__( 'Blog Archives', 'propane' ); ?></li>
		<?php } elseif ( is_search() ) { ?>
			<li><?php esc_html__( 'Search Results', 'propane' ); ?></li>
		<?php } ?>
		</ul>
	<?php
	}
	...
	public static function comment_nav() {
		...
		<nav class="navigation comment-navigation" role="navigation">
			<h2 class="screen-reader-text"><?php esc_html_e( 'Comment navigation', 'propane' ); ?></h2>
			<div class="nav-links">
				<?php

				$prev_link = get_previous_comments_link( esc_html__( 'Older Comments', 'propane' ) );

				if ( $prev_link ) {
					printf( '<div class="nav-previous">%s</div>', $prev_link ); // WPCS xss ok.
				}

				$next_link = get_next_comments_link( esc_html__( 'Newer Comments', 'propane' ) );
