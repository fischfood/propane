...
class Utilities {
	...
	public static function menu_fallback() {
		...
		$html .= sprintf(
			...
			wp_kses( __( 'Please assign a menu to the primary menu location under %1$s or %2$s the design.' ), 'propane', array( 'a' => array( 'href' ) ) ),
			self::sanitize_menu_fallback( 'nav-menus.php', __( 'Menu', 'propane' ) ),
			self::sanitize_menu_fallback( 'customize.php', __( 'Customize', 'propane' ) )
		);
		$html .= '</div>';
